...
Astra Design Language는 macOS의 차분하고 고급스러운 desktop 미학과 Samsung One UI 9.0의 부드럽고 직관적인 component 미학을 AstraOS 고유 방식으로 융합합니다. 시각 감성은 macOS의 정갈함, 여백, glass surface, 조용한 typography, premium app window 느낌을 강하게 참고하되, One UI 9.0의 큰 radius, soft card, 넉넉한 spacing, touch-friendly structure, smooth motion을 결합합니다. 목표는 macOS와 One UI가 떠오르지만 어느 쪽도 복제하지 않는, AstraOS만의 modern OS identity입니다.
...
Astra Design Language는 두 참고 감성을 다음 원칙으로 통합합니다.

| 원칙 | 설명 |
| --- | --- |
| Calm Premium Surface | macOS의 정갈함과 One UI의 부드러움을 결합한 차분한 surface |
| Soft Depth | glass, blur, shadow를 사용하되 과한 neon이나 cyber dashboard 느낌을 배제 |
| Trustworthy Security UI | 보안 상태는 과장하지 않고, 위험 작업은 명확하고 차분하게 표시 |
| Familiar Desktop Flow | desktop, app window, Shell Bar, Launcher, quick panel의 익숙한 흐름 유지 |
...
| Launcher | 시작 메뉴처럼 접근 가능하고 One UI식 넉넉한 spacing과 rounded component 감성 적용 |
| Control Center | 오른쪽 하단 compact quick panel, 짧고 명확한 설정 상태 |
| Notification Center | 오른쪽 slide-out 또는 compact panel, 과장 없는 알림 |
| Settings | 독립 app window, 큰 section과 명확한 row 구조 |
...
- Shell Bar, Launcher, Control Center, Notification Center component spec 작성
