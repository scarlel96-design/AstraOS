...
Phase 1-5d 기준 Astra Design Language는 macOS의 정갈한 desktop 미학, One UI 9.0의 부드럽고 직관적인 component 감성, Windows 11의 Mica/Acrylic depth와 Snap/Fluent 실용성을 AstraOS 고유 방식으로 융합합니다. 이 문서는 특정 제품 UI를 복제하기 위한 문서가 아니라, 세 디자인 언어의 핵심 감성을 AstraOS의 Linux desktop shell 맥락에 맞게 재해석하는 token 방향 문서입니다.
...
| Windows 11의 Mica/Acrylic depth | window와 launcher에 미세 texture, backdrop tint, clean shadow를 적용 |
| Windows 11의 Snap/Fluent 실용성 | snap preview, card hover, focus-visible, centered shell flow를 token화 |
...
- Windows 11 Start menu, taskbar, title bar button, logo를 token으로 복제하지 않습니다.
...
| `color.accent.warm` | 보조 warmth | macOS/One UI reference에서 온 peach/rose를 muted tone으로만 사용 |
| `color.accent.fluent` | depth accent | Windows 11 blue를 직접 복제하지 않고 AstraOS용 blue-violet 후보로 재해석 |
...
| `panel.mica.tint` | backdrop 위에 아주 미세한 color tint |
| `panel.mica.texture` | noise가 아니라 CSS gradient 기반의 subtle depth |
...
Phase 1-5d부터 radius는 3단계로 고정합니다.

| 단계 | 값 후보 | 용도 |
| --- | --- | --- |
| `radius.control` | 12px | button, input, small pill |
| `radius.card` | 18px | app tile, setting row, notification card |
| `radius.panel` | 28px | launcher, quick panel, app window, Shell Bar |
| `radius.full` | full | avatar, status dot, round icon |
...
## Snap / Multitasking 토큰 방향

| 토큰 | 값 후보 | 용도 |
| --- | --- | --- |
| `snap.preview.opacity` | 0.18-0.28 | window drag preview mock |
| `snap.preview.border` | accent muted border | edge snap guide |
| `snap.preview.radius` | `radius.panel`보다 약간 작게 | snap cell |
| `workspace.card.depth` | shallow Mica shadow | workspace overview card |

Phase 1-5d static mock은 Snap preview를 실제 drag/drop 기능이 아니라 시각 방향으로만 표현합니다. 실제 compositor-level snapping은 later phase work item입니다.
